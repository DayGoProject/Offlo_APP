...
`▰▰▰▰▱▱▱▱▱` **4 / 9 완료** — Google 로그인부터 대시보드 · 분석 기록까지 실데이터로 동작합니다. 다음은 AI 분석입니다.

| 단계 | 내용 | 상태 |
|:---:|------|:---:|
| **M1** | 저장소 · 기반 구축 (공유 코드 · 테마 · 폰트 · Expo 부팅) | ✅ **완료** |
| **M2** | 인증 (Firebase + AsyncStorage · Google 로그인) | ✅ **완료** |
| **M3** | API 연동 계층 (클라이언트 통합 · 타임아웃 · 재시도) | ✅ **완료** |
| **M4** | 내비게이션 + 대시보드 · 분석 기록 | ✅ **완료** |
...
- 디자인 토큰(`src/theme.ts`) — 웹과 같은 색 3개 체계 (M4에서 웹 디자인에 맞춰 다크 단일로 교체)
- 폰트 런타임 로드 (Expo Go에서도 동일하게 적용되도록 · M4에서 Pretendard + Familjen Grotesk로 교체)
